...
std::pair<std::size_t, std::string> findEnd(const std::string& _html, std::size_t pos, const std::string& endText = "{% END_IF %}") {
    auto ifCount = 1;
...
std::pair<std::size_t, std::unique_ptr<nodes::BaseNode>> ConditionalsParser::parse(std::string& _html, const HtmlCommand& cmd)
{
    ...
    auto [elsePos, elseCommand] = findEnd(_html, cmd.endPos, START_ELSE_TEXT);
